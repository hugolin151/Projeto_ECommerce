<?php
require_once __DIR__ . '/../../app/repositores/Conecta.php';

$sql = $pdo->query("SELECT * FROM produto"); 
$produtos = $sql->fetchAll(PDO::FETCH_ASSOC);

?>

<!DOCTYPE html>
<html lang="pt-br">

<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.8/dist/css/bootstrap.min.css" rel="stylesheet">

  <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.1/font/bootstrap-icons.css">

  <title>Document</title>
  <link rel="stylesheet" href="../../public/css/home.css">
</head>

<body>

  <!-- NAVBAR -->
  <nav class="navbar navbar-expand-lg navbar-dark custom-navbar">
    <div class="container-fluid">

      <a class="navbar-brand fw-bold" href="#">Logo</a>

      <form class="d-flex mx-auto search-wrapper">
        <input class="form-control search-input" type="search" placeholder="O que você está procurando?">
      </form>

      <div class="d-flex align-items-center">
        <a href="#" class="me-3">
          <i class="bi bi-cart3 fs-4 text-white"></i>
        </a>
        <a href="#" class="text-white text-decoration-none fw-semibold">Iniciar Sessão</a>
      </div>

    </div>
  </nav>

  <!-- BARRA SUPERIOR -->
  <div class="bg-light border-bottom">
    <div class="container d-flex justify-content-center gap-4 py-2 fw-semibold">

      <a href="#" class="text-dark text-decoration-none">Promoções</a>

      <!-- DROPDOWN HARDWARE -->
      <div class="dropdown">
        <a class="dropdown-toggle text-dark text-decoration-none" href="#" data-bs-toggle="dropdown">Hardware</a>
        <ul class="dropdown-menu">
          <li><a class="dropdown-item" href="#">Processadores</a></li>
          <li><a class="dropdown-item" href="#">Placas-mãe</a></li>
          <li><a class="dropdown-item" href="#">Placas de Vídeo</a></li>
          <li><a class="dropdown-item" href="#">Memórias RAM</a></li>
        </ul>
      </div>

      <!-- DROPDOWN PERIFÉRICOS -->
      <div class="dropdown">
        <a class="dropdown-toggle text-dark text-decoration-none" href="#" data-bs-toggle="dropdown">Periféricos</a>
        <ul class="dropdown-menu">
          <li><a class="dropdown-item" href="#">Teclados</a></li>
          <li><a class="dropdown-item" href="#">Mouses</a></li>
          <li><a class="dropdown-item" href="#">Headsets</a></li>
          <li><a class="dropdown-item" href="#">Mousepads</a></li>
        </ul>
      </div>

    </div>
  </div>

  <!-- PAINEL EXPANDIDO -->
  <div id="dropdown-content" class="dropdown-panel d-none">
    <div class="container py-4 text-white">

      <div id="conteudo-hardware" class="submenu-content d-none">
        <h4>Hardware</h4>
        <div class="d-flex gap-4 mt-3 flex-wrap">
          <a href="#" class="text-white">Processadores</a>
          <a href="#" class="text-white">Placas-mãe</a>
          <a href="#" class="text-white">Placas de Vídeo</a>
          <a href="#" class="text-white">Memória RAM</a>
        </div>
      </div>

      <div id="conteudo-perifericos" class="submenu-content d-none">
        <h4>Periféricos</h4>
        <div class="d-flex gap-4 mt-3 flex-wrap">
          <a href="#" class="text-white">Teclados</a>
          <a href="#" class="text-white">Mouses</a>
          <a href="#" class="text-white">Headsets</a>
          <a href="#" class="text-white">Mousepads</a>
        </div>
      </div>

    </div>
  </div>

  <!-- CARROSSEL -->
  <div id="carouselExample" class="carousel slide">
    <div class="carousel-inner">
      <div class="carousel-item active">
        <img src="computador-em-centro-de-dados-gerenciando-recursos-de-rede-usando-software-de-arvore-de-nos.jpg"
          class="d-block w-100" alt="...">
      </div>
      <div class="carousel-item">
        <img src="scripts-em-execucao-em-monitores-de-pcs-de-mesa-atualizando-servidores-em-centros-de-dados.jpg"
          class="d-block w-100" alt="...">
      </div>
      <div class="carousel-item">
        <img src="..." class="d-block w-100" alt="...">
      </div>
    </div>
    <button class="carousel-control-prev" type="button" data-bs-target="#carouselExample" data-bs-slide="prev">
      <span class="carousel-control-prev-icon"></span>
      <span class="visually-hidden">Previous</span>
    </button>
    <button class="carousel-control-next" type="button" data-bs-target="#carouselExample" data-bs-slide="next">
      <span class="carousel-control-next-icon"></span>
      <span class="visually-hidden">Next</span>
    </button>
  </div>


  <!-- CARDS (SEM ALTERAR NADA) -->
  <div class="container py-4">
    <h2 class="mb-4">Promoções</h2>
    <div class="row row-cols-1 row-cols-md-4 g-4">
  <?php foreach ($produtos as $produto):  ?>
      <div class="col">
         <a href="/Projeto_ECommerce/web/views/Produto/produto.php?id=<?php echo $produto['id']; ?>" class="text-decoration-none text-dark">
        <div class="card h-100">
          <img src="<?php echo $produto['img']; ?>" class="card-img-top" alt="<?php echo $produto['nome']; ?>">
          <div class="card-body">
            <h5 class="card-title"> <?php echo $produto['nome']; ?></h5>
            <p class="card-text">Valor: R$ <?php echo number_format($produto['preco'], 2, ',', '.'); ?></p>
          </div>
        </div>
  </a>
      </div>
<?php endforeach; ?>
      <div class="col">
        <div class="card h-100">
          <img src="<?php echo $produto['img']; ?>" class="card-img-top">
          <div class="card-body">
            <h5 class="card-title">Nome do Produto</h5>
            <p class="card-text">Valor: R$ 00,00</p>
          </div>
        </div>
      </div>

      <div class="col">
        <div class="card h-100">
          <img src="img/produto3.jpg" class="card-img-top" alt="Produto 3">
          <div class="card-body">
            <h5 class="card-title">Nome do Produto</h5>
            <p class="card-text">Valor: R$ 00,00</p>
          </div>
        </div>
      </div>

      <div class="col">
        <div class="card h-100">
          <img src="img/produto4.jpg" class="card-img-top" alt="Produto 4">
          <div class="card-body">
            <h5 class="card-title">Nome do Produto</h5>
            <p class="card-text">Valor: R$ 00,00</p>
          </div>
        </div>
      </div>

      <div class="col">
        <div class="card h-100">
          <img src="img/produto5.jpg" class="card-img-top" alt="Produto 5">
          <div class="card-body">
            <h5 class="card-title">Nome do Produto</h5>
            <p class="card-text">Valor: R$ 00,00</p>
          </div>
        </div>
      </div>

      <div class="col">
        <div class="card h-100">
          <img src="img/produto6.jpg" class="card-img-top" alt="Produto 6">
          <div class="card-body">
            <h5 class="card-title">Nome do Produto</h5>
            <p class="card-text">Valor: R$ 00,00</p>
          </div>
        </div>
      </div>

    </div>

    <!-- Periféricos -->
    <h2 class="mt-5 mb-4">Periféricos</h2>
    <div class="row row-cols-1 row-cols-md-4 g-4">

      <div class="col">
        <div class="card h-100">
          <img src="img/periferico1.jpg" class="card-img-top" alt="Periférico 1">
          <div class="card-body">
            <h5 class="card-title">Nome do Produto</h5>
            <p class="card-text">Valor: R$ 00,00</p>
          </div>
        </div>
      </div>

      <div class="col">
        <div class="card h-100">
          <img src="img/periferico2.jpg" class="card-img-top" alt="Periférico 2">
          <div class="card-body">
            <h5 class="card-title">Nome do Produto</h5>
            <p class="card-text">Valor: R$ 00,00</p>
          </div>
        </div>
      </div>

      <div class="col">
        <div class="card h-100">
          <img src="img/periferico3.jpg" class="card-img-top" alt="Periférico 3">
          <div class="card-body">
            <h5 class="card-title">Nome do Produto</h5>
            <p class="card-text">Valor: R$ 00,00</p>
          </div>
        </div>
      </div>

      <div class="col">
        <div class="card h-100">
          <img src="img/periferico4.jpg" class="card-img-top" alt="Periférico 4">
          <div class="card-body">
            <h5 class="card-title">Nome do Produto</h5>
            <p class="card-text">Valor: R$ 00,00</p>
          </div>
        </div>
      </div>

    </div>

    <!-- Peças -->
    <h2 class="mt-5 mb-4">Peças</h2>
    <div class="row row-cols-1 row-cols-md-4 g-4">

      <div class="col">
        <div class="card h-100">
          <img src="img/peca1.jpg" class="card-img-top" alt="Peça 1">
          <div class="card-body">
            <h5 class="card-title">Nome do Produto</h5>
            <p class="card-text">Valor: R$ 00,00</p>
          </div>
        </div>
      </div>

      <div class="col">
        <div class="card h-100">
          <img src="img/peca2.jpg" class="card-img-top" alt="Peça 2">
          <div class="card-body">
            <h5 class="card-title">Nome do Produto</h5>
            <p class="card-text">Valor: R$ 00,00</p>
          </div>
        </div>
      </div>

      <div class="col">
        <div class="card h-100">
          <img src="img/peca3.jpg" class="card-img-top" alt="Peça 3">
          <div class="card-body">
            <h5 class="card-title">Nome do Produto</h5>
            <p class="card-text">Valor: R$ 00,00</p>
          </div>
        </div>
      </div>

      <div class="col">
        <div class="card h-100">
          <img src="img/peca4.jpg" class="card-img-top" alt="Peça 4">
          <div class="card-body">
            <h5 class="card-title">Nome do Produto</h5>
            <p class="card-text">Valor: R$ 00,00</p>
          </div>
        </div>
      </div>

    </div>
  </div>

  <!-- FOOTER -->
  <footer class="text-white mt-5">
    <div class="container py-3">
      <div class="d-flex flex-column flex-md-row justify-content-between align-items-center">

        <div class="mb-3 mb-md-0">
          <a href="#" class="text-white me-3 fs-4"><i class="bi bi-facebook"></i></a>
          <a href="#" class="text-white me-3 fs-4"><i class="bi bi-twitter-x"></i></a>
          <a href="#" class="text-white me-3 fs-4"><i class="bi bi-instagram"></i></a>
          <a href="#" class="text-white fs-4"><i class="bi bi-whatsapp"></i></a>
        </div>

        <div class="text-white d-flex flex-wrap gap-5">
          <span>E-mail: exemplo@gmail.com</span>
          <span>Telefone: (11) 0000-0000</span>
          <span>Atendimento ao Cliente</span>
        </div>

      </div>
    </div>
  </footer>


  <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.8/dist/js/bootstrap.bundle.min.js"></script>
</body>

</html>
